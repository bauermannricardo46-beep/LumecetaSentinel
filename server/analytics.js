...
async function yahooChart(symbol) {
  ...
  for (let i = 0; i < timestamps.length; i++) {
    ...
    if (close > 0) rows.push({ date: dateKey(timestamps[i] * 1000), close });
  }
  return { symbol, rows };
}
...
async function paginateHistory(t212, endpoint, stopDateMs) {
  ...
  for (let page = 0; page < MAX_HISTORY_PAGES && path; page++) {
    ...
    const dates = pageItems.map(item => {
      const fill = item.fill || {};
      const order = item.order || item;
      return new Date(fill.filledAt || order.dateExecuted || order.createdAt || item.dateTime || 0).getTime();
...
function normalizeFill(item) {
  ...
  const qty = n(fill.quantity) || n(order.filledQuantity);
  const price = n(fill.price) || (n(order.filledValue) && n(order.filledQuantity) ? n(order.filledValue) / Math.abs(n(order.filledQuantity)) : 0);
  const side = String(order.side || '').toUpperCase();
  const filledAt = fill.filledAt || order.dateExecuted || order.createdAt;
...
function buildHoldingsHistory(orders, startMs) {
  const events = orders.map(normalizeFill).filter(e => e.ticker && e.date);
...
function priceOnOrBefore(rows, date) {
  for (let i = rows.length - 1; i >= 0; i--) if (rows[i].date <= date) return rows[i].close;
  return 0;
}
...
function returnFor(rows, days) {
  if (rows.length < 2) return 0;
  const end = rows[rows.length-1].close;
  const start = rows[Math.max(0, rows.length-1-days)].close;
  return start ? end/start - 1 : 0;
}
function maxDrawdown(rows) {
  let peak = 0, dd = 0;
  for (const r of rows) { peak = Math.max(peak, r.value); if (peak > 0) dd = Math.min(dd, r.value/peak - 1); }
  return dd;
}
...
function calculateAnalytics({ positions, priceSeries, portfolioHistory, transactions }) {
  ...
  const assetSignals = positions.map(p => {
    ...
    const r20 = returnFor(s,20), r60 = returnFor(s,60), r120 = returnFor(s,120);
    const momentum = .5*r20 + .3*r60 + .2*r120;
    let peak=0, drawdown=0;
    for (const r of s) { peak=Math.max(peak,r.close); if(peak) drawdown=Math.min(drawdown,r.close/peak-1); }
    return { ticker:p.ticker, weight:weights[p.ticker]||0, momentum, volatility, drawdown, returns:{r20,r60,r120} };
  });
  const weightedVol = assetSignals.reduce((s,a)=>s+(a.volatility||0)*a.weight,0);
  const weightedMomentum = assetSignals.reduce((s,a)=>s+(a.momentum||0)*a.weight,0);
  const maxWeight = Math.max(...Object.values(weights),0);
  const concentrationRisk = clamp((maxWeight-.2)/.6,0,1)*35;
  const volatilityRisk = clamp(weightedVol/.55,0,1)*30;
  const drawdownRisk = clamp(Math.abs(maxDrawdown(portfolioHistory))/.35,0,1)*20;
  const momentumRisk = clamp(-weightedMomentum/.25,0,1)*15;
  const riskScore = Math.round(clamp(concentrationRisk+volatilityRisk+drawdownRisk+momentumRisk,0,100));
  const momentumScore = Math.round(clamp(50+weightedMomentum*120,0,100));
  ...
  return {
    ...
    risk:{score:riskScore,label:riskScore>=70?'HIGH':riskScore>=45?'MODERATE':'LOW',concentration:Math.round(concentrationRisk/35*100),volatility:Math.round(volatilityRisk/30*100),drawdown:Math.round(drawdownRisk/20*100),momentumDrag:Math.round(momentumRisk/15*100)},
    momentum:{score:momentumScore,label:momentumScore>=65?'POSITIVE':momentumScore<=35?'NEGATIVE':'NEUTRAL',weightedReturn20d:weightedMomentum,assets:assetSignals},
    methodology:'Risk = concentration + annualized realized volatility + historical max drawdown + negative momentum penalty. Momentum = weighted 20/60/120-day price returns. Historical value is reconstructed from Trading 212 fills and daily market closes; no simulated prices are used.'
  };
}
...
async function buildAnalytics({t212,positions}) {
  ...
  const results=await Promise.all([...tickers].map(async ticker=>{
    try { return [ticker,(await yahooChart(yahooSymbol(ticker))).rows]; } catch(_) { return [ticker,null]; }
  }));
  for(const [ticker,series] of results) if(series?.length) priceSeries[ticker]=series;

  const dates=[]; const cursor=new Date(startMs); cursor.setUTCHours(0,0,0,0); const end=new Date(); end.setUTCHours(0,0,0,0);
  while(cursor<=end){ dates.push(dateKey(cursor)); cursor.setUTCDate(cursor.getUTCDate()+1); }
  const holdings=new Map(holdingHistory.initial), txs=transactions.filter(t=>t.dateTime||t.date).map(t=>({...t,date:t.dateTime||t.date})).sort((a,b)=>new Date(a.date)-new Date(b.date));
  const cash={value:0};
  ...
  for(const date of dates){
    ...
    let invested=0;
    for(const [ticker,qty] of holdings){ if(Math.abs(qty)<1e-9) continue; const rows=priceSeries[ticker]; if(rows) invested+=qty*priceOnOrBefore(rows,date); }
...
function createAnalyticsService({t212}){
  ...
  return { async get(positions,force=false){
    if(!force&&cache&&Date.now()-cache.at<CACHE_TTL_MS) return cache.data;
    if(running) return running;
    running=buildAnalytics({t212,positions}).then(data=>{cache={at:Date.now(),data};return data;}).finally(()=>{running=null;});
    return running;
  }};
}
