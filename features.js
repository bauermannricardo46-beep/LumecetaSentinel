(() => {
  ...
  function bindFeatures() {
    ...
    document.getElementById('alert-bell')?.addEventListener('click', () => { document.querySelectorAll('.view').forEach(v=>v.classList.remove('active-view')); document.getElementById('view-signals')?.classList.add('active-view'); document.querySelectorAll('.nav-item').forEach(b=>b.classList.toggle('active',b.dataset.view==='signals')); document.getElementById('page-title').textContent='Signals'; renderAlerts(); });
  }

  function addWatch() { const input=document.getElementById('watch-symbol'), symbol=(input?.value||'').trim().toUpperCase(); if(!symbol)return; if(!state.watchlist.includes(symbol))state.watchlist.push(symbol); saveWatch(); input.value=''; loadWatchlist(true); }
  function removeWatch(symbol) { state.watchlist=state.watchlist.filter(x=>x!==symbol); saveWatch(); loadWatchlist(true); }
  async function loadWatchlist(force=false) { const grid=document.getElementById('watch-grid'); if(!grid)return; grid.innerHTML='<div class="empty-state">Live-Marktdaten werden geladen…</div>'; try{const d=await api(`/watchlist?symbols=${encodeURIComponent(state.watchlist.join(','))}${force?'&force=1':''}`);renderWatchlist(d.items||[]);}catch(e){grid.innerHTML=`<div class="empty-state">${esc(e.message)}</div>`;} }
  function scoreLabel(s){return s>=70?'STRONG':s>=55?'POSITIVE':s>=40?'NEUTRAL':s>=25?'CAUTION':'WEAK';}
  function renderWatchlist(items){
    const grid=document.getElementById('watch-grid');if(!grid)return;document.getElementById('watch-count').textContent=`${items.length} ASSETS`;
  ...
  function renderHistoryRange(range='365'){
    const h=state.history;if(!h.length)return;const count=range==='all'?h.length:Math.min(h.length,Number(range)||365),slice=h.slice(-count),wrap=document.querySelector('.chart-wrap');if(!wrap)return;
    const vals=slice.map(x=>n(x.value)),min=Math.min(...vals),max=Math.max(...vals),span=max-min||1,w=900,height=210,pad=8,pts=vals.map((v,i)=>`${pad+i/(vals.length-1||1)*(w-pad*2)},${height-pad-((v-min)/span)*(height-pad*2)}`).join(' ');
    ...
    const first=n(slice[0].value),last=n(slice[slice.length-1].value),change=first?(last/first-1)*100:0,peak=Math.max(...vals),dd=peak?(last/peak-1)*100:0,stats=document.getElementById('history-stats');
    if(stats)stats.innerHTML=`<div><small>PERIOD</small><b>${range==='all'?'ALL':range+' DAYS'}</b></div><div><small>CHANGE</small><b class="${change>=0?'positive':'negative'}">${pct(change)}</b></div><div><small>END VALUE</small><b>${eur(last)}</b></div><div><small>FROM PEAK</small><b class="${dd>=-5?'positive':'negative'}">${pct(dd)}</b></div>`;
  }

  function renderEngine(data){
    state.analytics=data;state.history=data.history||[];const s=data.risk?.score??0,m=data.momentum?.score??0,composite=Math.round((100-s+m)/2);
    const summary=document.getElementById('engine-summary');if(summary)summary.innerHTML=`<div class="engine-score"><span>Sentinel Score</span><strong>${composite}</strong><small>composite</small></div><div class="engine-score"><span>Risk</span><strong>${s}</strong><small>${esc(data.risk?.label)}</small></div><div class="engine-score"><span>Momentum</span><strong>${m}</strong><small>${esc(data.momentum?.label)}</small></div><div class="engine-score"><span>Concentration</span><strong>${data.risk?.concentration??'—'}</strong><small>risk factor</small></div>`;
    const assets=document.getElementById('asset-signals');if(assets)assets.innerHTML=(data.momentum?.assets||[]).map(a=>{const ms=a.momentum==null?null:Math.round(50+a.momentum*120),risk=Math.round(Math.min(100,Math.max(0,(a.volatility||0)/.55*70+Math.abs(a.drawdown||0)/.35*30)));return`<div class="asset-signal"><div><b>${esc(a.ticker)}</b><span>${(a.weight*100).toFixed(1)}% weight</span></div><div><small>Momentum</small><strong>${ms??'—'}</strong></div><div><small>Risk</small><strong>${risk}</strong></div><div class="asset-bar"><i style="width:${Math.max(0,Math.min(100,ms??0))}%"></i></div></div>`}).join('')||'<div class="empty-state">Keine Asset-Marktserien verfügbar.</div>';
    const method=document.getElementById('engine-method');if(method)method.textContent=data.methodology||'';
    const factors=document.getElementById('portfolio-factors');if(factors)factors.innerHTML=`<div class="factor"><small>Concentration</small><b>${data.risk?.concentration??'—'}/100</b></div><div class="factor"><small>Volatility</small><b>${data.risk?.volatility??'—'}/100</b></div><div class="factor"><small>Drawdown</small><b>${data.risk?.drawdown??'—'}/100</b></div><div class="factor"><small>Momentum drag</small><b>${data.risk?.momentumDrag??'—'}/100</b></div>`;
    renderHistoryRange(document.querySelector('.history-controls button.active')?.dataset.range||'365');runAlerts(data);
  }

  function renderAlerts(){const list=document.getElementById('alerts-list');if(!list)return;if(!state.alerts.length){list.innerHTML='<div class="empty-state">Noch keine Alerts. Sentinel kann dich bei Risiko- oder Momentum-Schwellen warnen.</div>';updateBadge();return;}list.innerHTML=state.alerts.map((a,i)=>`<div class="alert-item"><div><b>${esc(a.type==='risk'?'Risk Alert':'Momentum Alert')}</b><span>${esc(a.symbol||'Portfolio')} · Schwelle ${a.threshold}</span></div><button data-alert="${i}" class="remove-alert">×</button></div>`).join('');list.querySelectorAll('.remove-alert').forEach(b=>b.addEventListener('click',()=>{state.alerts.splice(Number(b.dataset.alert),1);saveAlerts();renderAlerts();}));updateBadge();}
  ...
  function addAlert(){const type=prompt('Alert-Typ: risk oder momentum');if(!type)return;const threshold=prompt('Schwelle 0–100 eingeben (z. B. 60).');const t=n(threshold);if(!['risk','momentum'].includes(type.toLowerCase())||t<=0||t>100)return;state.alerts.push({type:type.toLowerCase(),threshold:t});saveAlerts();renderAlerts();}
  function runAlerts(data){const triggered=[];state.alerts.forEach(a=>{if(a.type==='risk'&&n(data.risk?.score)>=n(a.threshold))triggered.push(`Risk ${data.risk.score}/${a.threshold}`);if(a.type==='momentum'&&n(data.momentum?.score)>=n(a.threshold))triggered.push(`Momentum ${data.momentum.score}/${a.threshold}`);});if(triggered.length)console.info('Lumeceta Sentinel alerts:',triggered.join(' · '));updateBadge();}

  enhanceIndex();
  setTimeout(()=>{loadWatchlist(false);fetch(`${API}/analytics`).then(r=>r.ok?r.json():null).then(d=>d&&renderEngine(d)).catch(()=>{});renderAlerts();},50);
})();
