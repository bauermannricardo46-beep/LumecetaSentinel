(() => {
  ...
  function mount(){
    ...
    document.getElementById('asset-back').addEventListener('click',()=>window.showLumecetaView?.('watchlist'));
  }
  async function openAsset(ticker){
    mount(); window.showLumecetaView?.('asset'); document.getElementById('page-title').textContent=ticker;
    try{
